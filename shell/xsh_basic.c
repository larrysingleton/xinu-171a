 ...
 *
 * Version 1.0 (September 23, 2015)
 * Version 1.1 (October 15, 2016)
 ...
 *	DELAY ms			- Delays for the indicated time
 *	DIM var(size)[, ... ]		- Dimension an array
 *	DOS "command"			- Execute a DOS program -- XXX --
 *	END				- Terminate program with no message
 *	EXIT				- Terminate program (return to shell)
 ...
 *	OUT addr,expr			- store a byte in a memory location
 *	was OUT port,expr		- Write to I/O port
 *
...
/*---------------------------------------------------------------------*/
static char *version = "Xinu BASIC v 2.0\n";
...
#define	XDELAY	27
#define	BEEP	28
#define	DOS	29		/* change to allow Xinu shell escapes */
#define	OUT	30
...
static int32 prog_mode;				/* non-zero = interactive */
...
/* This is call by xsh_basic at program start.     */
/*-------------------------------------------------*/
...
/*************************************************************/
/* Sound a tone with frequency 'freq' for 'ms' milliseconds. */
/*************************************************************/
static void beep(unsigned freq, int ms)
{
    /* This will be added when Xinu provides appropriate support. */
}

/***********************************/
/* Invoke a shell to execute 'cmd' */
/***********************************/
static void system(char *cmd)
{
    /*----------------------------------------------------------------*/
    /* This will be added when Xinu provides appropriate support.     */
    /* Actually, the library will need to create a new shell process  */
    /* and pass it the string 'cmd' to execute. It's not the kernel's */
    /* responsibility to handle command lines!                        */
    /*----------------------------------------------------------------*/
}
...
/*************************************************************************/
/* Return non-zero if a character is ready to be read from the keyboard. */
/*************************************************************************/
static int kbtst(void)
{
    /* Keyboard raw-mode input test? */
    return 1;
}
...
/**************************************/
/* Test for the end of an expression. */
/**************************************/
...
#endif
{
	...
	/* If we matched all characters in the table entry, and */
	/* if it did not end with a letter or digit */
	if (!*cptr) {
...
static uint32 *address(void)
{
    ...
    } else {				/* nope, it's an integer variable */
	if (test_next('(')) {		/* Is it an array? */
...
static struct line_record *execute(char cmd)
{
	...
	case LIST:
		...
		i = get_num();
		if (get_next() == ',') {
		    if (isdigit(skip_blank())) j=get_num();
		    else j = -1;
		} else j=i;
	    }
	...
	case RUN1:		/* No clearing */
	    if (is_e_end(skip_blank())) runptr = pgm_start;
	    else runptr = find_line(eval_num());
...
input:	    if (ii == -1) fputs(sa1, stdout);
	...
	case DIM:
		...
		dim_vars[i] = (int *)allocate(dim_check[i] * 2);
		/* was: dim_vars[i] =
...
/*-------------------------------------------*/
/* Test to see if we're in direct only mode. */
...
static int32 get_value(void)
{
	...
	switch(c) {
		...
		eval_sub();
		if (!expr_type)		/* String result is required */
		    error(4);
